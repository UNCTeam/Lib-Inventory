...
import fr.teamunc.inventory_unclib.models.inventories.UNCInventoryHolder;
...
public class UNCInventoryAdapter implements JsonSerializer<UNCInventory>, JsonDeserializer<UNCInventory> {
    ...
    @Override
    public UNCInventory deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        if (inventoryType.equals(InventoryType.CHEST)) {
            ...
            if (jsonObject.has("uncHolder")) {
                inventoryHolder = gson.fromJson(jsonObject.get("uncHolder"), UNCInventoryHolder.class);
            }
            inventory = Bukkit.createInventory(inventoryHolder, size, title);
        }
        inventory.setContents(contents);
        return UNCInventory.builder(key)
                .title(title)
                .inventory(inventory)
                .build();
    }
    ...
    @Override
    public JsonElement serialize(UNCInventory src, Type typeOfSrc, JsonSerializationContext context) {
        Gson gson = new GsonBuilder().create();
        ...
        if (src.getInventory().getHolder() instanceof UNCInventoryHolder) {
            jsonObject.addProperty("uncHolder", gson.toJson((src.getInventory().getHolder())));
        }
        return jsonObject;
    }
}
